...
public class PQNode
{
  ...
  static final int LABEL_EMPTY = 0;
  static final int LABEL_PARTIAL = 1;
  static final int LABEL_FULL = 2;
  static final int TYPE_PNODE = 0;
  static final int TYPE_QNODE = 1;
  protected static final int TYPE_DNODE = 3;

  int label; // 0 - empty, 1 - partial, 2 - full
  protected int type; // 0 - P-node, 1 - Q-Node, 2 D-Node (Direction)

  boolean blocked;
  boolean queued;
  int pertinentChildCount; // The number of pertinent children before the reduction
  int pertinentLeafCount; // The number of pertinent leaf decendants after reduction
  ...
  PQNode parent = null; // not valid for marked interior q-node children, or null for root.

  // SIBLING ACCESS VARIABLES
  protected PQNodePair siblings = null; // only valid for children of q-nodes
  PQNode left = null; // for p-node children only
  PQNode right = null;// for p-node children only
  PQNode fullLeft = null; // for full children only
  PQNode fullRight = null; // for full children only
  PQNode partialLeft = null; // for partial children only
  PQNode partialRight = null; // for partial children only

  // CHILDREN ACCESS VARIABLES
  int childCount; // number of child nodes, for p-nodes only
  int fullChildCount; // number of full child nodes.
  int partialChildCount; // number of partial child nodes (max is 2)
  PQNodePair endMostChildren = null; // only valid for q-nodes
  PQNode childAccessNode = null; // child node to use to access children of p-nodes
  ...
  int debugGetLabelValue() { return label; }
  Object debugGetDataValue() { return data; }
  PQNode debugGetLeftChild() { return left; }
  PQNode debugGetRightChild() { return right; }
  PQNode debugGetFullLeftChild() { return fullLeft; }
  PQNode debugGetFullRightChild() { return fullRight; }
  PQNode debugGetPartialLeftChild() { return partialLeft; }
  PQNode debugGetPartialRightChild() { return partialRight; }
  int debugGetFullChildCount() { return fullChildCount; }
  int debugGetPartialChildCount() { return partialChildCount; }
  PQNode debugGetFullChildAccessNode() { return fullChildAccessNode; }
  PQNodePair debugGetEndMostChildrenValue() { return endMostChildren; }
  boolean debugIsQueuedValue() { return queued; }
  boolean debugIsBlockedValue() { return blocked; }
  void debugSetDepthValue(int value) { depth = value; }
  void debugSetSubLeafCountValue(int value) { subLeafCount = value; }
  int debugGetSubLeafCountValue() { return subLeafCount; }

  public int getNumChildren() throws Exception
  {
    return PQNodeStateSupport.getNumChildren(this);
  }

  public int getNumEmptyChildren() throws Exception
  {
    return PQNodeStateSupport.getNumEmptyChildren(this);
  }

  // convert this node to a q-node.
  public void convertToQNode() throws Exception
  {
    PQNodeStateSupport.convertToQNode(this);
  }

  // convert this node to a p-node.
  public void convertToPNode() throws Exception
  {
    PQNodeStateSupport.convertToPNode(this);
  }

  public void convertToDNode() throws Exception
  {
    PQNodeStateSupport.convertToDNode(this);
  }

  // label this node as a full node, updating its parent.
  public void labelAsFull() throws Exception
  {
    PQNodeStateSupport.labelAsFull(this);
  }

  // label this node as a partial node, updating its parent.
  public void labelAsPartial() throws Exception
  {
    PQNodeStateSupport.labelAsPartial(this);
  }

  // label this node as a partial node, updating its parent.
  public void labelAsEmpty() throws Exception
  {
    PQNodeStateSupport.labelAsEmpty(this);
  }

  // returns whether or not the current node has any children.
  public boolean hasChildren()
  {
    return PQNodeStateSupport.hasChildren(this);
  }

  // returns all the children of this node in a vector, used only for displaying the tree.
  Vector getAllChildren() throws Exception
  {
  ...
  {
    return PQNodeStateSupport.hasOnlyOneChild(this);
  }

  public boolean hasOnlyTwoChildren()
  {
    return PQNodeStateSupport.hasOnlyTwoChildren(this);
  }

  // the default behaviour of the clear method is to clear the parent after this node.
  public void clear() throws Exception
  {
    PQNodeStateSupport.clear(this);
  }
  ...
  {
    PQNodeStateSupport.clear(this, recurse);
  }

  // returns an informative (debug) String representation of this node's values.
  public String infoString()
  {
    return PQNodeDebugSupport.infoString(this);
  }

  // returns a String representation of this node.
  public String toString()
  {
    return PQNodeDebugSupport.toDisplayString(this);
  }
  ...
  {
    PQNodeDebugSupport.printStructure(this);
  }
  ...
  {
    return PQNodeDebugSupport.countSubLeaves(this, parent_depth);
  }
  ...
  {
    return PQNodeDebugSupport.countSubDeletedNodes(this);
  }
  ...
  {
    return PQNodeDebugSupport.countSubDeletedNodes(this, deletedNodes);
  }
  ...
  {
    return PQNodeDebugSupport.countSubNodes(this);
  }
}
